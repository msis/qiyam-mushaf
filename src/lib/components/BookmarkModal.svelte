...
>
	<div class="bg-gray-800 rounded-xl p-6 max-w-md w-full mx-4 shadow-2xl border border-gray-700 max-h-[80vh] flex flex-col">
		<div class="flex justify-between items-center mb-4">
			<h2 id="modal-title" class="text-xl font-bold text-amber-100">{t('bookmarks.title')}</h2>
			<button
				onclick={onClose}
		...
		{#if bookmarks.length === 0}
			<BookmarkEmpty />
		{:else}
			<div class="flex-1 overflow-y-auto space-y-2" role="list" aria-label={t('bookmarks.title')}>
